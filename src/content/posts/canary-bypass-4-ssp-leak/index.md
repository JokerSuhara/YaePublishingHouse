---
title: "Canary绕过专题(4)——SSP泄露"
published: 2025-12-01
description: "利用SSP泄露获取flag 机制讲解 SSP 是编译器为了防止栈溢出攻击加入的一种保护机制 当程序运行时检测到Canary被破坏，会触发 stackchkfail 函数，准备终止程序并报错。 关键特性： 报错机制： 在 glibc 2.25"
image: "../Canary.png"
tags: [CTF, Pwn]
category: CTF
draft: false
slug: canary-bypass-4-ssp-leak
---# 利用SSP泄露获取flag
...
* **第二个** `%s` **(**__**libc**_**argv[0])：**它原本意图是输出当前运行的程序名
...
`buf堆块+Header`只有**0x40**大小，但 `read`允许我们写入**0x41**的数据，这意味着我们可以将 `ptr`函数指针的**最低位字节**覆盖掉
...
>   * `0`: 属于主线程
>
> 在写脚本的时候，填充完 `buf`数据段后，需要伪造下一个堆块的**Header**，否则会触发***Error in `./canary4': double free or corruption (out): 堆地址 ***
>
> ~~**理论上有fork()的时候也可以用这种方法泄露堆地址**~~
...
***0x5646e7ed1010 + 0x48 = 0x5646e7ed1058***
...
***0x7fffffffddc8 - 0x7fffffffdca0 = 0x128 = 296***
